import { Component, OnInit, HostListener, ViewChild, ElementRef, AfterViewInit, ChangeDetectorRef, Output, EventEmitter } from '@angular/core';
...
@Component({
  selector: 'app-schedules-dashboard-calendar',
  templateUrl: './calendar.component.html',
  styleUrls: ['./calendar.component.scss']
})
export class DashboardSchedulesCalendarComponent implements OnInit {
  ...
  @Output() changeToNavigation: EventEmitter<any> = new EventEmitter();
  ...
  setEventData() {
    ...
    const budgetVerificationEvent = this.budgetVerificationEvents?.budget_verification.map(ele => {
      ...
      let inspection_finalized = ele?.budget_verification?.info?.inspection_finalized || false;
      if (ele?.budget_verification?.info?.inspection_start_time && ele?.budget_verification?.info?.inspection_end_time && !inspection_finalized) {
        return {
          id: ele._id,
    ...
    const drawsEvent = this.budgetVerificationEvents?.draws.map(ele => {
      let inspection_finalized = ele?.draw?.inspection?.inspection_finalized || false;
      if (ele?.draw?.inspection?.inspection_start_time && ele?.draw?.inspection?.inspection_end_time && !inspection_finalized) {
        return {
          id: ele._id,
          type: 'Draws',
          ...ele,
    ...
    const inspectionEvent = this.budgetVerificationEvents?.inspections.map(ele => {
      let inspection_finalized = ele?.inspections?.inspection_finalized || false;
      if (ele?.inspections?.inspection_start_time && ele?.inspections?.inspection_end_time && !inspection_finalized) {
        return {
          id: ele._id,
          type: 'Inspection',
          ...ele,
    ...
    this.total_events = this.total_budget_verification + this.total_draw + this.total_inspection;
      ...
      this.modalService.open(this.openViewModal, { size: 'sm', centered: true, backdrop: 'static', keyboard: false }).result
        .then((result) => {
          if (result === 'navigation') {
            this.openNavigationView(events);
          } else {
            openPreviewModal();
          }
  ...
  openNavigationView(inspections) {
    this.modalService.dismissAll();
    this.changeToNavigation.emit({ date: this.viewDate, inspections });
  }
